...
int main(int argc, char** argv){
    ...
    for (int i = 0 ; i < tests_int.size();i++){
        ...
        priority_queue<int, vector<int>, std::less<int>> maxHeap (tests_int[i].begin(), tests_int[i].end());
        printHeap(maxHeap);
    }
    ...
    for (int i = 0 ; i < tests_int.size();i++){
        ...
        priority_queue<int, vector<int>, std::greater<int>> minHeap (tests_int[i].begin(), tests_int[i].end());
        printHeap(minHeap);
    ...
    for (int i = 0 ; i < tests_vec.size() ; i++){
        ...
        priority_queue<P, vector<P>, decltype(comp_less)> pq(tests_vec[i].begin(), tests_vec[i].end(), comp_less); 
        printHeap(pq);
    }
    printSep();
    // 由小到大
    for (int i = 0 ; i < tests_vec.size() ; i++){
        ...
        priority_queue<P, vector<P>, decltype(comp_greater)> pq(tests_vec[i].begin(), tests_vec[i].end(), comp_greater);
        printHeap(pq);
    }
