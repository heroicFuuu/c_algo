...
然後，我們還需要瞭解一下 printf 由於類型不匹配，所以，會把 float 直接轉成 double(What happens to a float variable when %d is used in a printf?)，注意，12.5 的 float 和 double 的記憶體二進制完全不一樣。別忘了在x86晶片下使用是的反位元組序，高位位元組和低位字位要反過來。所以：
